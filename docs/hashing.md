...
To access the hashing parameters, you can call `auth.DefaultSaltParameters()`.
...
To use a secret pepper, you can call the `PepperInit()` function. Make sure to store your secret key somewhere safe, because the library stores it in memory and frees it once the program is executed and does not manage your secret keys.
